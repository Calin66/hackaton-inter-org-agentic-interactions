...
def load_tariff() -> Dict[str, float]:
    ...
    if not d:
        raise RuntimeError(
            "No suitable price dict found in tariff.py (expects a dict with numeric values)."
        )
    return d


def best_tariff_match(
    proc_free_text: str, choices: List[str], cutoff: float = 0.6
) -> Optional[str]:
...
def build_initial_invoice(
    extracted: Dict[str, Any], tariff: Dict[str, float]
) -> Dict[str, Any]:
    matched = []
...
def pretty_invoice(invoice: Dict[str, Any]) -> str:
    lines = []
    lines.append(
        f"Patient: {invoice.get('patient name','-')}  SSN: {invoice.get('patient SSN','-')}"
    )
    lines.append(
        f"Hospital: {invoice.get('hospital name','-')}  Date: {invoice.get('date of service','-')}"
    )
...
def add_procedure_exact(
    invoice: Dict[str, Any], tariff: Dict[str, float], exact_name: str
) -> str:
...
def add_procedure_free_text(
    invoice: Dict[str, Any], tariff: Dict[str, float], free_text: str
) -> str:
